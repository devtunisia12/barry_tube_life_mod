...
****/
//=========================================================
// Sophia atahane (passive lab worker)
//=========================================================
...
class CSofcut : public CTalkMonster
{
public:
	void Spawn(void);
	void Precache(void);
...
LINK_ENTITY_TO_CLASS(monster_sofcut, CSofcut);

TYPEDESCRIPTION	CSofcut::m_SaveData[] =
{
	DEFINE_FIELD(CSofcut, m_painTime, FIELD_TIME),
	DEFINE_FIELD(CSofcut, m_healTime, FIELD_TIME),
	DEFINE_FIELD(CSofcut, m_fearTime, FIELD_TIME),
};

IMPLEMENT_SAVERESTORE(CSofcut, CTalkMonster);
...
Schedule_t	soflFollow[] =
{
	{
		tlsofFollow,
		ARRAYSIZE(tlsofFollow),
		bits_COND_NEW_ENEMY |
...
Schedule_t	soflFollowScared[] =
{
	{
		tlsofFollowScared,
		ARRAYSIZE(tlsofFollowScared),
		bits_COND_NEW_ENEMY |
		bits_COND_HEAR_SOUND |
		bits_COND_LIGHT_DAMAGE |
		bits_COND_HEAVY_DAMAGE,
...
Schedule_t	sofFaceTargetScared[] =
{
	{
		tlsofFaceTargetScared,
		ARRAYSIZE(tlsofFaceTargetScared),
		bits_COND_HEAR_SOUND |
		bits_COND_NEW_ENEMY,
		bits_SOUND_DANGER,
		"FaceTargetScared"
...
Task_t	tlsofStopFollowing[] =
{
	{ TASK_CANT_FOLLOW, (float)0 },
};

Schedule_t	slsofStopFollowing[] =
{
	{
		tlsofStopFollowing,
		ARRAYSIZE(tlsofStopFollowing),
		0,
		0,
		"StopFollowing"
	},
...
Schedule_t	sofFaceTarget[] =
{
	{
		tlsofFaceTarget,
		ARRAYSIZE(tlsofFaceTarget),
...
Schedule_t	sofSciPanic[] =
{
	{
		tlsofSciPanic,
		ARRAYSIZE(tlsofSciPanic),
		0,
		0,
		"SciPanic"
	},
...
Schedule_t	slsofIdleSciStand[] =
{
	{
		tlsofIdleSciStand,
		ARRAYSIZE(tlsofIdleSciStand),
		bits_COND_NEW_ENEMY |
		bits_COND_LIGHT_DAMAGE |
		bits_COND_HEAVY_DAMAGE |
		bits_COND_HEAR_SOUND |
...
Schedule_t	slsofScientistCover[] =
{
	{
		tlsofScientistCover,
		ARRAYSIZE(tlsofScientistCover),
		bits_COND_NEW_ENEMY,
		0,
		"ScientistCover"
	},
...
Schedule_t	slsofScientistHide[] =
{
	{
		tlsofScientistHide,
		ARRAYSIZE(tlsofScientistHide),
		bits_COND_NEW_ENEMY |
		bits_COND_HEAR_SOUND |
		bits_COND_SEE_ENEMY |
		bits_COND_SEE_HATE |
...
Schedule_t	slsofScientistStartle[] =
{
	{
		tlsofScientistStartle,
		ARRAYSIZE(tlsofScientistStartle),
		bits_COND_NEW_ENEMY |
		bits_COND_SEE_ENEMY |
		bits_COND_SEE_HATE |
		bits_COND_SEE_FEAR |
...
DEFINE_CUSTOM_SCHEDULES(CSofcut)
{
	soflFollow,
		sofFaceTarget,
		slsofIdleSciStand,
...
IMPLEMENT_CUSTOM_SCHEDULES(CSofcut, CTalkMonster);


void CSofcut::DeclineFollowing(void)
{
	Talk(10);
...
Activity CSofcut::GetStoppedActivity(void)
{
	if (m_hEnemy != NULL)
		return ACT_EXCITED;
...
void CSofcut::StartTask(Task_t *pTask)
{
	switch (pTask->iTask)
	{
	case TASK_SAY_HEAL:
...
void CSofcut::RunTask(Task_t *pTask)
{
	switch (pTask->iTask)
	{
	case TASK_RUN_PATH_SCARED:
...
//=========================================================
void CSofcut::HandleAnimEvent(MonsterEvent_t *pEvent)
{
	switch (pEvent->event)
	{
...
// Spawn
//=========================================================
void CSofcut::Spawn(void)
{
	Precache();

	SET_MODEL(ENT(pev), "models/sofcut.mdl");
	...
	MonsterInit();
	SetUse(&CSofcut::FollowerUse);
}

//=========================================================
// Precache - precaches all resources this monster needs
//=========================================================
void CSofcut::Precache(void)
{
	PRECACHE_MODEL("models/sofcut.mdl");
...
int CSofcut::TakeDamage(entvars_t *pevInflictor, entvars_t *pevAttacker, float flDamage, int bitsDamageType)
{
...
// DeathSound 
//=========================================================
void CSofcut::DeathSound(void)
{
	PainSound();
}


void CSofcut::Killed(entvars_t *pevAttacker, int iGib)
{
	SetUse(NULL);
...
void CSofcut::SetActivity(Activity newActivity)
{
	int	iSequence;
...
Schedule_t* CSofcut::GetScheduleOfType(int Type)
{
	Schedule_t *psched;
	...
	case SCHED_TARGET_FACE:
		...
		if (psched == slIdleStand)
			return sofFaceTarget;	// override this for different target face behavior
		else
			return psched;
	...
	case SCHED_CANT_FOLLOW:
		return slsofStopFollowing;

	case SCHED_PANIC:
		return sofSciPanic;

	case SCHED_TARGET_CHASE_SCARED:
		return soflFollowScared;

	case SCHED_TARGET_FACE_SCARED:
		return sofFaceTargetScared;
	...
	case SCHED_IDLE_STAND:
		...
		if (psched == slIdleStand)
			return slsofIdleSciStand;
		else
			return psched;

	case SCHED_HIDE:
		return slsofScientistHide;

	case SCHED_STARTLE:
		return slsofScientistStartle;
...
Schedule_t *CSofcut::GetSchedule(void)
{
	...
	case MONSTERSTATE_COMBAT:
		if (HasConditions(bits_COND_NEW_ENEMY))
			return slsofFear;					// Point and scream!
		if (HasConditions(bits_COND_SEE_ENEMY))
			return slsofScientistCover;		// Take Cover

		if (HasConditions(bits_COND_HEAR_SOUND))
			return slTakeCoverFromBestSound;	// Cower and panic from the scary sound!

		return slsofScientistCover;			// Run & Cower
		break;
	}
...
MONSTERSTATE CSofcut::GetIdealState(void)
{
	switch (m_MonsterState)
	{
	case MONSTERSTATE_ALERT:
...
int CSofcut::FriendNumber(int arrayNumber)
{
