...
void CShotgunA::Precache(void)
{
	...
	PRECACHE_SOUND("weapons/sshotgun_reload.wav"); // sawnoff reload
...
void CShotgunA::Reload(void)
{
	if (m_pPlayer->m_rgAmmo[m_iPrimaryAmmoType] <= 0 || m_iClip == SHOTGUNA_MAX_CLIP)
		return;
	DefaultReload(SHOTGUNA_MAX_CLIP, SHOTGUNA_RELOAD, 1.5);
...
void CShotgunA::WeaponIdle(void)
{
	...
	{
		// play pumping sound
		EMIT_SOUND_DYN(ENT(m_pPlayer->pev), CHAN_ITEM, "weapons/sshell3.wav", 1, ATTN_NORM, 0, 95 + RANDOM_LONG(0, 0x1f));
		m_flPumpTime = 0;
	}
	...
	if (m_flTimeWeaponIdle <  UTIL_WeaponTimeBase())
	{
		...
		else if (m_fInSpecialReload != 0)
		{
			...
			{
				// play cocking sound
				EMIT_SOUND_DYN(ENT(m_pPlayer->pev), CHAN_ITEM, "weapons/sshell2.wav", 1, ATTN_NORM, 0, 95 + RANDOM_LONG(0, 0x1f));
				m_fInSpecialReload = 0;
